...
def get_argparser() -> argparse.ArgumentParser:
	...
	parser.add_argument("--sl_mode1", type=str, required=True)
	parser.add_argument("--sl_mode2", type=str, required=True)
	parser.add_argument("--change_epoch", type=int, default=50)
...
def add_args_str(base_model_name: str, args: argparse.Namespace) -> str:
	model_name = f"{base_model_name}_{args.model_type}_{args.sl_mode1}_to_{args.sl_mode2}_{args.change_epoch}_{args.lam}_{args.tau}_{args.mom}"
	if args.sl_mode1 in {"mf", "tef"} or args.sl_mode2 in {"mf", "tef"}:
